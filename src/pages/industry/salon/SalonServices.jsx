...
export default function SalonServices() {
    ...
    const fetchServices = async () => {
        try {
            const response = await apiClient.get('/salon-services');
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Salon Services</h1>
                </div>
            ...
            {services.length === 0 ? (
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-12 text-center">
                    ...
                    <p className="text-slate-500 dark:text-slate-400">Add salon services to start booking appointments.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {services.map((service) => (
                        <div key={service.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-sm text-slate-500">{service.duration_minutes || 30} min</span>
                            </div>
                        </div>
                    ))}
                </div>
